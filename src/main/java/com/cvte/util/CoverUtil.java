...
public class CoverUtil {
	...
	public static void refreshLabel(List<Double> coverList) {
		for(int i = 0; i < coverList.size(); i++) {
			...
			Constant.CoverList.get(i).setText("" + coverList.get(i));
		}
	}
...
//				}
	...
	private static double calcuteCover(int commonLabel, int label, int flag) {
		...
		if(flag == 0) {
			...
			CircleData solepan1 = LabelUtil.getSoleLabelData(commonLabel, "shipan");
			CircleData solepan2 = LabelUtil.getSoleLabelData(label, "shipan");
			cover = calCover(solepan1, solepan2);
		}
		else if(flag == 1) {
			System.out.println("计算视杯cover");
			CircleData solebei1 = LabelUtil.getSoleLabelData(commonLabel, "shibei");
			CircleData solebei2 = LabelUtil.getSoleLabelData(label, "shibei");
...
//		}
	...
	private static double calCover1(CircleData sole1, CircleData sole2) {
		...
		for(double i = 0; i < 530; i = i+0.1) {
			for(double j = 0; j < 530; j = j + 0.1) {
				...
				double tmp2 = ((i - x2)*cos2 - (j - y2)*sin2)*((i - x2)*cos2 - (j - y2)*sin2)/(a2*a2) + 
						((i - x2)*sin2 + (j - y2)*cos2)*((i - x2)*sin2 + (j - y2)*cos2)/(b2*b2);
				if(tmp1 <= 1) {
					len3++;
				}
				if(tmp1 <= 1 && tmp2 <= 1) {
